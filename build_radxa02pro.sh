...
VENDOR_PATH="${LINEAGE_ROOT}/vendor/radxa/radxa02pro"
DEVICE_PATH="${LINEAGE_ROOT}/device/radxa/radxa02pro"
...
if [[ -d "${VENDOR_PATH}" ]]; then
    cd "$VENDOR_PATH" && git add -A && git commit -m "radxa02pro: Update bootloader image prebuilts"
fi
